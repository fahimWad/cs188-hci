    ...
    <div className="sidebar bg-secondary-1 w-[300px] h-screen p-8 flex flex-col box-ring gap-4 fixed x-500 y-0 z-100">
      ...
      <div className="flex flex-col gap-4 overflow-visible">
        {filtered.map((card: FlashcardData) => {
          const cardKey: string | number = card.id ?? card.front;
            ...
            >
              ...
              <div
                className={`-translate-y-1/4 w-full flex flex-row items-center justify-center ${
                  hoveredId === cardKey ? "opacity-100" : "opacity-0"
              ...
              >
                <DeleteButton
                  ...
                  onClick={() => {
                    setCards((prev) =>
                      prev.filter((c) => (c.id ?? c.front) !== cardKey)
                    );
                    setHighlights((prev) =>
                      prev.filter((h) => String(h.flashcardID) !== cardKey)
                    );
