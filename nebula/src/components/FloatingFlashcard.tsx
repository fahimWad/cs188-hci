...
const FloatingFlashcard: React.FC<FloatingFlashcardProps> = ({
  ...
  return (
    <div className="fixed bottom-4 left-1/2 transform -translate-x-1/2 translate-x-12 bg-secondary-2 text-white-50 p-4 rounded shadow-lg z-50 ring-primary-3 ring-4 w-[400px] h-[300px] [perspective:1000px] flex flex-col">
      {/* Flashcard header */}
      ...
      >
        ...
        <div
          className="absolute inset-0 flex items-center justify-center
                    bg-secondary-2 border border-primary-3 rounded-lg shadow
                    text-xl font-bold
                    [backface-visibility:hidden] overflow-auto"
        >
          {flashcard.front.length > 0 ? 
            (
            <p className="text-center p-2">{flashcard.front}</p>
            ) : (
            <p className="text-center p-2">Select text from PDF</p>
          )}
        </div>
        {/* Flashcard back */}
        <div
          className="absolute inset-0 flex items-center justify-center
                    bg-secondary-2 bg-primary-3 rounded-lg shadow
                    text-xl font-bold
        ...
        >
          {flashcard.back.length > 0 ? 
            (
            <p className="text-center p-2">{flashcard.back}</p>
            ) : (
            <p className="text-center p-2">Select text from PDF</p>
          )}
        </div>
