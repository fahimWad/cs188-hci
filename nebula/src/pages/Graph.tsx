...
const FlowCanvas: React.FC<FlowCanvasProps> = ({
  ...
  const onNodesChange: OnNodesChange = useCallback(
    (changes) =>
      setNodes(
        (nds) => applyNodeChanges(changes, nds as Node[]) as AllNodeTypes[]
      ),
    [setNodes]
  );
  ...
  const modifyAnnotation = (
    ...
    setNodes((prevNodes) =>
      prevNodes.map((node) => {
        ...
        return {
          ...node,
          data: { ...node.data, content: data.content, icon: data.icon },
        };
      })
    );
  };
  ...
  const showPopupCard = useCallback(
    (id: string, type: string) => {
      if (type == "Node") {
        ...
        const foundCard = nodes.find((node): node is AnnotationNodeType => {
